...
defmodule Prodigy.Server.Service.Logon do
  ...
  # The three historically-shipped client versions. Kept for reference
  # only - the logon path no longer enforces a whitelist (newer builds
  # are permitted; see the test "logon accepts a client version outside
  # the historically-shipped set").
  ...
  def make_response_payload({Status.SUCCESS, user, _session_id}) do
    ...
    last_logon_date = val_or_else(User.last_logon_date(user), "        ")
    last_logon_time = val_or_else(User.last_logon_time(user), "     ")
  ...
  defp get_user(user_id, password) do
    ...
    if user == nil do
      ...
      # The changeset hashes on write, so user.password is always a pbkdf2 digest
      # except for rows that may have been inserted before that change landed.
      # Fall back to hashing-then-comparing in that case so legacy data still works.
      encrypted_pw =
        if String.starts_with?(user.password, "$pbkdf2-sha512$") do
          user.password
        else
          Pbkdf2.hash_pwd_salt(user.password)
        end

      if Pbkdf2.verify_pass(password, encrypted_pw) do
  ...
  def handle(
    ...
    result =
      with true <- version_ok(version),
        ...
        case SessionManager.create_session(user, status, version, session_opts) do
          {:ok, db_session} ->
...
#        {:enroll_other, user} -> {Status.ENROLL_OTHER, user}
    ...
    case result do
      {Status.SUCCESS, user, session_id} ->
        ...
        {:ok, %{context | user: user, session_id: session_id, rs_version: version, auth_timeout: nil}, response}
