...
defmodule Prodigy.Server.Service.LogonLogoff.Test do
  ...
  test "Authentication timeout canceled after un-enrolled subscriber logon", context do
    ...
    |> change
    |> put_assoc(:users, [
      %User{id: "AAAA12A", profile: %{"0157" => "F"}}
      |> User.changeset(%{password: "foobaz"})
    ])
    |> Repo.insert!()
  ...
  test "Authentication timeout reset after logoff (for re-logon)", context do
    ...
    |> change
    |> put_assoc(:users, [
      %User{id: "AAAA12D", profile: %{"0157" => "M"}, date_enrolled: @today}
  ...
  test "logon accepts a client version outside the historically-shipped set", context do
    %Household{id: "AAAA12", enabled_date: @today}
    |> change
    |> put_assoc(:users, [
      %User{id: "AAAA12A", profile: %{"0157" => "F"}, date_enrolled: @today}
    ...
    {:ok, response} = logon(context.router_pid, "AAAA12A", "foobaz", "06.03.11")
  ...
  test "logon succeeds for un-enrolled member", context do
    ...
    |> change
    |> put_assoc(:users, [
      %User{id: "AAAA12B", profile: %{"0157" => "F"}}
  ...
  test "logon fails for deleted user", context do
    ...
    |> change
    |> put_assoc(:users, [
      %User{id: "AAAA12C", profile: %{"0157" => "F"}, date_deleted: @today}
  ...
  test "disabled household prohibits logon", context do
    ...
    |> change
    |> put_assoc(:users, [
      %User{id: "BBBB12A", profile: %{"0157" => "M"}, date_enrolled: @today}
      |> User.changeset(%{password: "test"})
    ])
    |> Repo.insert()
...
#    Process.sleep(4000)
  ...
  test "user with unlimited concurrency can have multiple sessions", _context do
    ...
    |> change
    |> put_assoc(:users, [
      %User{id: "AAAA12A", profile: %{"0157" => "F"}, date_enrolled: @today, concurrency_limit: 0}
      |> User.changeset(%{password: "test"})
    ])
    |> Repo.insert!()
  ...
  test "user with concurrency_limit=2 allows exactly 2 sessions", _context do
    ...
    |> change
    |> put_assoc(:users, [
      %User{id: "AAAA13A", profile: %{"0157" => "M"}, date_enrolled: @today, concurrency_limit: 2}
      |> User.changeset(%{password: "test"})
    ])
    |> Repo.insert!()
  ...
  test "user session is marked abnormal when server terminates unexpectedly", _context do
    ...
    |> change
    |> put_assoc(:users, [
      %User{id: "AAAA14A", profile: %{"0157" => "F"}, date_enrolled: @today}
      |> User.changeset(%{password: "test"})
    ])
    |> Repo.insert!()
  ...
  test "concurrency_limit nil defaults to 1", _context do
    ...
    |> change
    |> put_assoc(:users, [
      %User{id: "AAAA16A", profile: %{"0157" => "F"}, date_enrolled: @today, concurrency_limit: nil}
      |> User.changeset(%{password: "test"})
    ])
    |> Repo.insert!()
